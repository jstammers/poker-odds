...
impl HoleCardsScreen {
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect) {
        ...
        let title = Paragraph::new(vec![
            ...
            Line::from(Span::styled(
                format!(
                    "  Enter {} hole cards (e.g. Ah, Td, 2c)",
                    self.variant.hole_card_count()
                ),
                Theme::dim(),
            )),
        ]);
        ...
        let card_block = Block::default()
            .borders(Borders::ALL)
            .border_style(if !self.editing_opponents {
        ...
        // Input prompt
        let prompt_style = if !self.editing_opponents {
            Theme::highlight()
        } else {
            Theme::dim()
        };
        let prompt = if self.input.all_complete() {
            Paragraph::new(Line::from(Span::styled(
                "  All cards entered. Press Tab to edit opponents or Enter to continue.",
                Theme::dim(),
            )))
        } else {
            ...
            let hint = match &active.state {
                ...
                crate::tui::widgets::card_input::InputState::AwaitingSuit(r) => format!(
                    "  Got rank {}. Enter suit: c(lubs), d(iamonds), h(earts), s(pades)",
                    r.to_char()
                ),
        ...
        // Opponents
        let opp_style = if self.editing_opponents {
            Theme::border_focused()
        } else {
            Theme::border()
        };
        ...
        let opp_text = Paragraph::new(Line::from(vec![
            ...
            Span::styled(
                format!("{}", self.opponent_count),
                Theme::highlight().add_modifier(Modifier::BOLD),
            ),
            Span::styled(
                format!(
                    " opponent{}",
                    if self.opponent_count == 1 { "" } else { "s" }
                ),
                Theme::normal(),
            ),
            Span::styled(
