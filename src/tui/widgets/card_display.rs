...
use ratatui::{buffer::Buffer, layout::Rect, style::Modifier, text::Span};
...
fn render_card(card: Option<Card>, buf: &mut Buffer, area: Rect) {
    ...
    // Border
    let border_style = if card.is_some() {
        Theme::border()
    } else {
        Theme::dim()
    };
    ...
    buf[(area.x, area.y + 1)]
        .set_char('│')
        .set_style(border_style);
    buf[(area.x + 1, area.y + 1)]
        .set_char(rank_str.chars().next().unwrap_or(' '))
        .set_style(style);
    buf[(area.x + 2, area.y + 1)].set_char(' ').set_style(style);
    buf[(area.x + 3, area.y + 1)]
        .set_char(suit_char)
        .set_style(style);
    buf[(area.x + 4, area.y + 1)]
        .set_char('│')
        .set_style(border_style);
    ...
    if area.height >= 3 {
        buf[(area.x, area.y + 2)]
            ...
            .set_style(border_style);
        for i in 1..4 {
            buf[(area.x + i, area.y + 2)]
                .set_char('─')
                .set_style(border_style);
        }
        buf[(area.x + 4, area.y + 2)]
            .set_char('┘')
            .set_style(border_style);
    }
}
...
pub fn card_span(card: Card) -> Span<'static> {
    ...
    Span::styled(
        text,
        Theme::suit_style(card.suit).add_modifier(Modifier::BOLD),
    )
}
